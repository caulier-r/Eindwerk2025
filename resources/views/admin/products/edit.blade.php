                    ...
                    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="4"
                                      class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md">{{ old('description', $product->description) }}</textarea>
                            @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category" id="category"
                                    class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md">
                                <option value="FiveM Plugin" {{ old('category', $product->category) == 'FiveM Plugin' ? 'selected' : '' }}>FiveM Plugin</option>
                                <option value="Discord Bot" {{ old('category', $product->category) == 'Discord Bot' ? 'selected' : '' }}>Discord Bot</option>
                        ...
                        <div class="mb-4" id="frameworkSection" style="display: none;">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                @php
                                    ...
                                    $selectedFrameworks = old('frameworks', $product->frameworks ?? []);
                                @endphp
                        ...
                        <div class="mb-4">
                            ...
                            @if($product->image)
                                <div class="mt-2 mb-4">
                                    <img src="{{ asset('images/' . $product->image) }}" alt="{{ $product->name }}" class="h-24 w-auto">
